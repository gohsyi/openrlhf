...
def batch_generate(args):
    ...
    for prompts, info in pbar:
        ...
        for _ in range(N):
            ...
            for messages, prefix, output, solution in zip(info["messages"], prefixes, outputs, info["solutions"]):
                output_dataset.append({
                    "messages": messages + [{"role": "assistant", "content": output[len(prefix):]}],
                    "solution": solution,
                    "score": reward_model(output[len(prefix):], solution)[0]
                })
    ...
    strategy.print(
        "Mean:", np.mean([data["score"] for data in output_dataset]), 
        "Std:", np.std([data["score"] for data in output_dataset])
    )
...
def math(args):
    ...
    rewards = []
    for data in dataset:
        split_ans = data["messages"][-1]["content"].split('The answer is: ')
        if len(split_ans) > 1:
            ans = split_ans[-1]
            extract_ans_temp = ans.split('.\n')[0]
            extract_ans_temp = extract_ans_temp.strip()
            if len(extract_ans_temp)>0 and extract_ans_temp[-1] == '.':
                ...
                extract_ans = extract_ans_temp
            extract_ans = extract_ans.strip()
            rewards.append(1.0 if is_equiv(extract_ans, data["solution"]) else 0.0)
        else:
            rewards.append(0.0)

        dataset.add_column("reward", rewards)
        with jsonlines.open(args.output_path, mode="w") as writer:
