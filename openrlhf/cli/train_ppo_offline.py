...
from openrlhf.trainer import OfflineTrainer
...
def train(args):
    ...
    get_tokenizer(args.initial, initial_model.model, "left", strategy)
    ...
    if args.pretrain_data:
        ...
        pretrain_dataset = SFTDataset(
            pretrain_data.select(range(min(len(pretrain_data), args.max_epochs * len(offline_dataset)))),
            tokenizer,
            pretrain_max_len,
            strategy,
            pretrain_mode=True,
    ...
    # prepare dataloader
    offline_dataloader = strategy.setup_dataloader(
        offline_dataset, 
        args.micro_rollout_batch_size, 
        pin_memory=True, 
        shuffle=True,
        collate_fn=partial(offline_dataset.collate_fn, padding=False),
    )
    if args.pretrain_data:
    ...
    if args.rollout_batch_size < len(offline_dataset):
        args.rollout_batch_size = len(offline_dataset)

    # configure scheduler
    num_update_steps_per_episodes = len(offline_dataset) * args.n_samples_per_prompt // args.train_batch_size * args.max_epochs
    ...
    actor_scheduler = get_scheduler(
        ...
        scheduler_specific_kwargs={"min_lr": args.actor_learning_rate},
    )
    ...
    critic_scheduler = get_scheduler(
        ...
        num_warmup_steps=math.ceil(max_steps * 0.03),
        num_training_steps=max_steps,
        scheduler_specific_kwargs={"min_lr": args.critic_learning_rate},
    )
    ...
    trainer.fit(args, offline_dataloader, pretrain_dataloader, consumed_samples, num_update_steps_per_episodes)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_kl_coef2", type=float, default=0.0, help="KL penalty for reweighting")
    ...
    parser.add_argument("--initial", type=str, default=None, help="HF model name or path")
    ...
    parser.add_argument("--weight_key", type=str, default=None)
    parser.add_argument("--reward_key", type=str, default=None)
