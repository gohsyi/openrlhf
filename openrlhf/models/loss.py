...
class WeightedPolicyLoss(nn.Module):
    ...
    def forward(
        ...
        surr2 = ratio.clamp(1 - self.clip_eps, 1 + self.clip_eps) * advantages
        loss = -torch.min(surr1, surr2)
        loss = masked_mean(loss, action_mask, dim=-1) * weights
        loss = loss.mean()
        return loss


class WeightedPolicyLossv2(nn.Module):
    """
    ...
    def forward(
        ...
        ratio = (log_probs - old_log_probs).exp().sum(1)
        surr1 = ratio * advantages
        surr2 = ratio.clamp(1 - self.clip_eps, 1 + self.clip_eps) * advantages
        loss = -torch.min(surr1, surr2) * weights
        return loss.mean()
