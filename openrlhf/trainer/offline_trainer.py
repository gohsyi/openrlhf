...
from openrlhf.models import Actor, GPTLMLoss, WeightedPolicyLoss, ValueLoss
from openrlhf.models.utils import masked_mean
from openrlhf.utils.distributed_sampler import DistributedSampler

from .ppo_utils import AdaptiveKLController, Experience, FixedKLController, OfflineExperienceMaker, NaiveReplayBuffer
...
class OfflineTrainer(ABC):
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        self.actor_loss_fn = WeightedPolicyLoss(eps_clip)
        ...
        self.kl_ctl2 = FixedKLController(getattr(self.args, "init_kl_coef2", 0.0))

        self.experience_maker = OfflineExperienceMaker(
            actor,
            critic,
            reward_model,
            initial_model,
    ...
    def fit(
        ...
        self.offline_dataloader = offline_dataloader
        ...
        for episode in range(start_episode, args.num_episodes):
            if isinstance(self.offline_dataloader.sampler, DistributedSampler):
                self.offline_dataloader.sampler.set_epoch(
                    episode, consumed_samples=0 if episode > start_episode else consumed_samples
                )
            pbar = tqdm(
                range(self.offline_dataloader.__len__()),
                desc=f"Episode [{episode + 1}/{args.num_episodes}]",
                disable=not self.strategy.is_rank_0(),
            )

            for prompt_ids_lens, inputs, _, infos in self.offline_dataloader:
                for _ in range(self.n_samples_per_prompt):
                    experience = self.experience_maker.make_experience(
                        ...
                        infos["weights"], 
                        infos["rewards"], 
                        use_cache=True, **self.generate_kwargs
                    )
