...
class PPOTrainer(ABC):
    ...
    def evaluate(self, eval_dataloader):
        ...
        for rand_prompts, _ in eval_dataloader:
            inputs = tokenize_fn(self.tokenizer, rand_prompts, self.prompt_max_len, device="cuda")
            # generate seq
        ...
        return {
            "reward": self.strategy.all_reduce(r.mean(), "mean"),
        }

    def save_logs_and_checkpoints(self, args, global_step, step_bar, eval_dataloader=None, logs_dict={}, client_states={}):
        ...
        if eval_dataloader and global_step % args.eval_steps == 0:
            eval_logs_dict = self.evaluate(eval_dataloader)
            eval_logs = {
                "eval/%s" % k: v
                for k, v in {
                    **eval_logs_dict,
