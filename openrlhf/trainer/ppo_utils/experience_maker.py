...
class NaiveExperienceMaker(ABC):
    ...
    def make_experience(
        ...
        **generate_kwargs
        ...
        # rewards
        r = self.reward_model(self.tokenizer.batch_decode(sequences[:, -num_actions:], skip_special_tokens=True), solutions)
        r = torch.as_tensor(r, device="cuda").float()
...
class OfflineExperienceMaker(NaiveExperienceMaker):
    ...
    def make_experience(
        ...
        **generate_kwargs
        ...
        # |<---------- prompt ----------->|<-------- answer ------->|
        input_ids = [seq.squeeze(0) for seq in input_ids]
        output_lens = [len(seq) - input_len for input_len, seq in zip(input_lens, input_ids)]
        max_input_len, max_output_len = max(input_lens), max(output_lens)

        pad_token_id, eos_token_id = self.tokenizer.pad_token_id, self.tokenizer.eos_token_id
        sequences = []
        for input_len, output_len, seq in zip(input_lens, output_lens, input_ids):
            left_padding = [pad_token_id] * (max_input_len - input_len)
            right_padding = [pad_token_id] * (max_output_len - output_len)
            assert seq[-1] == eos_token_id
            sequences.append(left_padding + seq.tolist() + right_padding)
        ...
        # rewards
        r = torch.tensor(rewards, device="cuda").float()
    ...
    def make_experience(
        ...
        **generate_kwargs
        ...
        rewards = self.reward_model(
            self.tokenizer.batch_decode(sequences.cpu(), skip_special_tokens=True),
            solutions
        )
        ...
        r = torch.as_tensor(rewards, device=device)
